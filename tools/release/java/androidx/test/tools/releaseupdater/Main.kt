...
class Main {
  ...
  companion object {
    ...
    fun main(args: Array<String>) {
      val versionsToUpdate = parseVersionsToMap(
        Main::class.java.getClassLoader().getResourceAsStream("build_extensions/axt_versions.bzl")
      )

      val releasedVersions = parseVersionsToMap(
        Main::class.java.getClassLoader().getResourceAsStream("build_extensions/axt_released_versions.bzl")
      )

      println(ReleaseUpdater().validateAndMakeRemoveDepsCommand(releasedVersions, versionsToUpdate))
    }
  }
}
